...
namespace ShowroomMaster
{
    public partial class frmSales : Form
    {
        ...
        private void purchGridFill()
        {
            ...
            SqlInfo.con.Open();
            string saleQuery = "Select STOCK_PAYMENT.ORDER_ID, CAR.CAR_NAME,PERSONEL.PERSONEL_NAME,MANUFACTURER.MANUFACTURER_NAME,MANUF_ORDER.BILL, STOCK_PAYMENT.PAYMENT_DATE from STOCK_PAYMENT inner join MANUF_ORDER on STOCK_PAYMENT.ORDER_ID = MANUF_ORDER.ORDER_ID inner join CAR on MANUF_ORDER.CAR_ID = CAR.CAR_ID inner join PERSONEL on MANUF_ORDER.PERSONEL_ID = PERSONEL.PERSONEL_ID inner join MANUFACTURER on MANUF_ORDER.MANUFACTURER_ID = MANUFACTURER.MANUFACTURER_ID";
            SqlCommand viewCarCmd = new SqlCommand(saleQuery, SqlInfo.con);
            ...
            for (int i = 0; i < (carData.Tables[0].Rows.Count); i++)
            {
                ...
                string PERSONELName = Convert.ToString(carData.Tables[0].Rows[i].ItemArray[2]);
                ...
                pushData.Cells[2].Value = PERSONELName;
        ...
        private void salesGridFill()
        {
            ...
            SqlInfo.con.Open();
            string purchQuery = "Select SELL_PAYMENT.ORDER_ID, CAR.CAR_NAME, PERSONEL.PERSONEL_NAME, CUSTOMER.CUSTOMER_NAME,CUSTOMER_ORDER.BILL, SELL_PAYMENT.PAYMENT_DATE from SELL_PAYMENT inner join CUSTOMER_ORDER on SELL_PAYMENT.ORDER_ID = CUSTOMER_ORDER.ORDER_ID inner join CAR on CUSTOMER_ORDER.CAR_ID = CAR.CAR_ID inner join PERSONEL on CUSTOMER_ORDER.PERSONEL_ID = PERSONEL.PERSONEL_ID inner join CUSTOMER on CUSTOMER_ORDER.CUSTOMER_TC = CUSTOMER.CUSTOMER_TC";
            SqlCommand viewCarCmd = new SqlCommand(purchQuery, SqlInfo.con);
            SqlDataAdapter viewCarAdapter = new SqlDataAdapter(viewCarCmd);
            DataSet carData = new DataSet();
            viewCarAdapter.Fill(carData);
